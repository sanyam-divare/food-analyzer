...
FODMAP: rate based on actual portion size. Small amounts of high-FODMAP foods are often LOW/MEDIUM. Cooking reduces FODMAP levels. Prefer LOW/MEDIUM unless clearly a large portion.
...
def stream_gut_with_gemini(image_base64, mime_type="image/jpeg", patient_profile=None):
    """
    Stream Gemini gut analysis as SSE.
    Yields text chunks then final done event.
    No food detection mid-stream — simple and reliable.
    """
    if not GEMINI_API_KEY:
    ...
    try:
        with requests.post(url, json=payload, stream=True, timeout=90) as r:
            if r.status_code != 200:
                yield f'data: {json.dumps({"error": f"Gemini error {r.status_code}"})}\n\n'
                return
        ...
        # Parse complete result and send done event
        ...
        try:
            ...
            result = {"error": "Failed to parse response", "foods": []}
        yield f'data: {json.dumps({"done": result})}\n\n'

    except Exception as e:
        yield f'data: {json.dumps({"error": str(e)})}\n\n'


def stream_gut_with_claude(image_base64, mime_type="image/jpeg", patient_profile=None):
    """
    Stream Claude gut analysis as SSE.
    Same simple format as stream_gut_with_gemini.
    """
    if not CLAUDE_API_KEY:
        yield f'data: {json.dumps({"error": "Missing CLAUDE_API_KEY"})}\n\n'
        return
    ...
    try:
        with requests.post(
            ...
            if r.status_code != 200:
                yield f'data: {json.dumps({"error": f"Claude error {r.status_code}"})}\n\n'
                return
