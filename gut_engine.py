...
def stream_gut_with_gemini(image_base64, mime_type="image/jpeg", patient_profile=None):
    ...
    if not GEMINI_API_KEY:
        yield f'data: {_json.dumps({"error": "Missing GEMINI_API_KEY"})}\n\n'
        return
    ...
    try:
        ...
        with requests.post(url, json=payload, stream=True, timeout=90) as r:
            ...
            for line in r.iter_lines():
                ...
                if raw.strip() == "[DONE]":
                    break
                try:
                    token = (_json.loads(raw).get("candidates", [{}])[0]
        ...
        try:
            ...
            result = {"foods": foods_done, "overall_gut_score": 0,
                      "overall_gut_notes": "Parse error — partial results shown"}

        yield f'data: {_json.dumps({"done": result})}\n\n'
...
def stream_gut_with_claude(image_base64, mime_type="image/jpeg", patient_profile=None):
    """Stream Claude gut analysis - same SSE event format as stream_gut_with_gemini."""
    import json as _json
    if not CLAUDE_API_KEY:
        yield f'data: {_json.dumps({"error": "Missing CLAUDE_API_KEY"})}\n\n'
        return
            ...
            json={"model": "claude-haiku-4-5-20251001",
                  ...
                  "messages": [{"role": "user", "content": [
                      {"type": "image", "source": {"type": "base64",
                       "media_type": mime_type, "data": image_base64}},
                      {"type": "text", "text": build_gut_prompt(patient_profile)}
                  ]}]},
            stream=True, timeout=90
        ) as r:
            if r.status_code != 200:
                yield f'data: {_json.dumps({"error": f"Claude error {r.status_code}"})}\n\n'
                return
